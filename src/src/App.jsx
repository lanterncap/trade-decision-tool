...
function Journal({ entries, onUpdate, onDelete, onBack }) {
  ...
  const stats = {
    ...
    winRate: entries.filter(e => e.pnl !== null).length > 0
      ? (entries.filter(e => parseFloat(e.pnl) > 0).length / entries.filter(e => e.pnl !== null).length * 100).toFixed(0)
      : null,
    avgPnl: entries.filter(e => e.pnl !== null).length > 0
      ? (entries.filter(e => e.pnl !== null).reduce((sum, e) => sum + parseFloat(e.pnl), 0) / entries.filter(e => e.pnl !== null).length).toFixed(1)
      : null
  };
...
export default function TradeDecisionTool() {
